...
SHMUP_UIManager::SHMUP_UIManager(SHMUP_GameManager* gameManager) : UIManager((GameManager*)gameManager)
{
	...
	bool scrollX = atoi(xmlDoc.FirstChildElement("map")->FirstChildElement("GameMap")->FirstChildElement("scrollX")->GetText()) != 0;
	bool scrollY = atoi(xmlDoc.FirstChildElement("map")->FirstChildElement("GameMap")->FirstChildElement("scrollY")->GetText()) != 0;
	float speedX = (float)atof(xmlDoc.FirstChildElement("map")->FirstChildElement("GameMap")->FirstChildElement("speedX")->GetText());
	float speedY = (float)atof(xmlDoc.FirstChildElement("map")->FirstChildElement("GameMap")->FirstChildElement("speedY")->GetText());
	...
	m_backgroundVector.push_back(new BackgroundSprite(mapWidth, mapHeight, mapPosX, mapPosY, mapPathW, this));
	static_cast<BackgroundSprite*>(m_backgroundVector.back())->SetScrollX(scrollX);
	static_cast<BackgroundSprite*>(m_backgroundVector.back())->SetScrollY(scrollY);
	...
	//m_foregroundVector.push_back(new Sprite(g_windowWidth, g_windowHeight / 4, 0, 0, this));
	//m_foregroundVector[0]->Init(L"img\\MenuBackground.dds");

	//// Map zoom buttons and bind event delegates
	//m_foregroundVector.push_back(new UIButton(64, 64, g_windowWidth - 64, g_windowHeight - g_windowHeight / 3, 0.8f, 1.0f, 0.5f, L"img\\zoomin.dds", this));
	...
	//m_foregroundVector.push_back(new UIButton(64, 64, g_windowWidth - 64, g_windowHeight - g_windowHeight / 3 - 128, 0.8f, 1.0f, 0.5f, L"img\\zoomout.dds", this));
